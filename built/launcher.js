...
class TaskResults {
	...
	reportSummary() {
		...
		this.results_.forEach(result => {
			let shortTestPath = result.specs['0'].match(/Protractor.(.*)$/)[1];
			...
			if (result.failedCount) {
				...
				spaceCount = spaceCount - shortName.length;
			}
			...
			if (result.failedCount) {
				...
				logger.info(`${shortName} passed${emptyString}('${shortTestPath}')`);
			}
		});
...
let initFn = function(configFile, additionalConfig) {
	...
	// Run beforeLaunch
	helper
		.runFilenameOrFn_(config.configDir, config.beforeLaunch)
		.then(() => {
			return q
				.Promise((resolve, reject) => {
					...
					if (config.getMultiCapabilities && typeof config.getMultiCapabilities === 'function') {
						if (config.multiCapabilities.length || config.capabilities) {
							logger.warn(
								'getMultiCapabilities() will override both capabilities ' + 'and multiCapabilities'
							);
						}
		...
		.then(() => {
			...
			let cleanUpAndExit = exitCode => {
				return helper.runFilenameOrFn_(config.configDir, config.afterLaunch, [exitCode]).then(
					returned => {
			...
			if (totalTasks > 1) {
				forkProcess = true;
				if (config.debug) {
					throw new exitCodes_1.ConfigError(
						logger,
