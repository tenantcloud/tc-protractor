...
class Local extends driverProvider_1.DriverProvider {
	...
	addDefaultBinaryLocs_() {
		if (!this.config_.seleniumServerJar) {
			logger.debug(
				'Attempting to find the SeleniumServerJar in the default ' + 'location used by webdriver-manager'
			);
			try {
				...
				throw new exitCodes_1.BrowserError(
					logger,
		...
		if (!fs.existsSync(this.config_.seleniumServerJar)) {
			throw new exitCodes_1.BrowserError(
				logger,
				'No selenium server jar found at ' +
					this.config_.seleniumServerJar +
		...
		if (this.config_.capabilities.browserName === 'chrome') {
			if (!this.config_.chromeDriver) {
				logger.debug(
					'Attempting to find the chromedriver binary in the default ' + 'location used by webdriver-manager'
				);
				try {
					let updateJson = path.resolve(SeleniumConfig.getSeleniumDir(), 'update-config.json');
					let updateConfig = JSON.parse(fs.readFileSync(updateJson).toString());
		...
		if (this.config_.capabilities.browserName === 'firefox') {
			if (!this.config_.geckoDriver) {
				logger.debug(
					'Attempting to find the gecko driver binary in the default ' + 'location used by webdriver-manager'
				);
				try {
					let updateJson = path.resolve(SeleniumConfig.getSeleniumDir(), 'update-config.json');
					let updateConfig = JSON.parse(fs.readFileSync(updateJson).toString());
			...
			if (!fs.existsSync(this.config_.geckoDriver)) {
				if (fs.existsSync(this.config_.geckoDriver + '.exe')) {
					...
					throw new exitCodes_1.BrowserError(
						logger,
						'Could not find gecko driver at ' +
							this.config_.geckoDriver +
							". Run 'webdriver-manager update' to download binaries."
					);
				}
			}
	...
	setupDriverEnv() {
		...
		this.server_
			.start(this.config_.seleniumServerStartTimeout)
			.then(url => {
