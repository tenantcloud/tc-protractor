...
class TaskScheduler {
	...
	constructor(config) {
		...
		this.allSpecs = configParser_1.ConfigParser.resolveFilePatterns(
			configParser_1.ConfigParser.getSpecs(config),
			false,
			config.configDir
		).filter(path => {
		...
		config.multiCapabilities.forEach(capabilities => {
			let capabilitiesSpecs = this.allSpecs;
			if (capabilities.specs) {
				let capabilitiesSpecificSpecs = configParser_1.ConfigParser.resolveFilePatterns(
					capabilities.specs,
					false,
					config.configDir
				);
			...
			if (capabilities.exclude) {
				let capabilitiesSpecExcludes = configParser_1.ConfigParser.resolveFilePatterns(
	...
	nextTask() {
		for (let i = 0; i < this.taskQueues.length; ++i) {
			if (clipboard.getAllSpecs() === 0) {
				let countAllSpecs = 0;
				for (let i = 0; i < this.taskQueues.length; i++) {
					countAllSpecs += this.taskQueues[i].specLists.length;
				}
				clipboard.setAllSpecs(countAllSpecs);
			}
