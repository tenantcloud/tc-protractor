...
class Runner extends events_1.EventEmitter {
	...
	createBrowser(plugins, parentBrowser) {
		...
		let browser_ = new browser_1.ProtractorBrowser(
			driver,
		...
		browser_.ready = browser_.ready
			...
			.then(() => {
				return driver
					.manage()
					.timeouts()
					.setScriptTimeout(initProperties.allScriptsTimeout || 0);
			})
			.then(() => {
				return browser_;
			});
		...
		browser_.restart = () => {
			...
			if (browser_.controlFlowIsEnabled()) {
				...
				return this.driverprovider_
					.quitDriver(browser_.driver)
					.then(replaceBrowser)
	 ...
	 */
	shutdown_() {
		return driverProviders_1.DriverProvider.quitDrivers(
	...
	run() {
		...
		return q(this.ready_)
			...
			.then(() => {
				...
				return browser_.ready.then(browser_.getSession).then(
					session => {
						logger.debug(
